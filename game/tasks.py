    ...
    for _ in range(batch_count):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                parse_and_save_bgg_xml(response.content)
                current_id = end_id
                time.sleep(3)
...
def parse_and_save_bgg_xml(xml_data):
    ...
    for item in root.findall('item'):
        ...
        defaults = {
            ...
            'year_published': _get_val(item, 'yearpublished'),
            'min_players': _get_val(item, 'minplayers'),
            'max_players': _get_val(item, 'maxplayers'),
            'playing_time': _get_val(item, 'playingtime'),
